...
import { Clock, Ticket,Timer,ExternalLink } from 'lucide-react';
...
const Itinerary = () => {
  ...
  const data = [
      ...
      content: (
        <div>
          <div className="grid md:grid-cols-2 gap-6">
            {TRIP_DATA.trip_plan?.hotels?.map((hotel, index) => (
    ...
    // Insert each itinerary day as its own timeline item
    ...((TRIP_DATA.trip_plan?.itinerary || []).map((day) => ({
      title: `Day ${day.day}`,
      content: (
        <div>
          <p className="mb-2 text-sm text-neutral-600">Best Time: {day.best_time_to_visit_day}</p>
